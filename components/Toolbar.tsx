...
interface ToolbarProps {
  ...
  onLogLatexAnnotations: () => void; // New prop
  isDisabled: boolean;
  currentPage: number;
  totalPages: number;
...
const Toolbar: React.FC<ToolbarProps> = ({
  ...
  onGetTutoringHint,
  onLogLatexAnnotations, // New prop
  isDisabled,
  currentPage,
  totalPages,
  onPageChange,
  ...
  return (
    <div className="w-20 bg-gray-800 p-3 flex flex-col space-y-3 print:hidden shadow-lg">
      ...
      />
      <ToolButton
        label="Log LaTeX Annotations"
        icon={<CodeIcon />}
        isActive={false} // This is an action, not a selectable tool state
        onClick={onLogLatexAnnotations}
        {...commonButtonProps}
      />
      ...
      {selectedTool === Tool.PEN && !isDisabled && (
        <div className="p-2 bg-gray-700 rounded-md space-y-2">
          ...
          <input
            ...
            aria-label="Pen line width slider"
          />
        </div>
      )}
         ...
         {totalPages > 0 && (
          <div className="flex flex-col items-center text-xs text-gray-400 bg-gray-700 p-2 rounded-md">
            ...
            <div className="flex mt-1">
              <button onClick={() => onPageChange(Math.max(1, currentPage - 1))} disabled={currentPage <= 1 || isDisabled} className="px-2 py-1 bg-gray-600 hover:bg-gray-500 rounded-l disabled:opacity-50" aria-label="Previous page">-</button>
              <button onClick={() => onPageChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage >= totalPages || isDisabled} className="px-2 py-1 bg-gray-600 hover:bg-gray-500 rounded-r disabled:opacity-50" aria-label="Next page">+</button>
            </div>
          </div>
        )}
        <div className="flex flex-col items-center text-xs text-gray-400 bg-gray-700 p-2 rounded-md">
           <label htmlFor="zoomLevel" className="text-xs text-gray-400">Zoom: {Math.round(zoomLevel*100)}%</label>
            <select 
              ...
              aria-label="Zoom level selector"
            >
...
const SelectIcon = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75v4.5m0-4.5h-4.5m4.5 0L15 9m5.25 11.25v-4.5m0 4.5h-4.5m4.5 0L15 15" /></svg>; // Changed to heroicons/arrows-up-down-left-right
const CodeIcon = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5" /></svg>;
